...
sudo wget --no-check-cert 'https://bitbucket.org/teamfluffee/fluffees-server-setup/raw/master/Debian/Debian-7/tigervncserver.txt' &> /dev/null
...
sed -i "s/$vncPort = 5900/$vncPort = $vncport - 1/g" /usr/local/bin/vncserver
sed -i "s/sockaddr_in(5900/sockaddr_in($vncport - 1/g" /usr/local/bin/vncserver
sudo wget --no-check-cert 'https://bitbucket.org/teamfluffee/fluffees-server-setup/raw/master/Debian/Debian-7/xstartup.txt' &> /dev/null
