...
yum -y update
yum -y install epel-release sudo wget curl nano tigervnc-server gnome-system-monitor firefox
...
wget --no-cookies --no-check-certificate --header "Cookie: gpw_e24=http%3A%2F%2Fwww.oracle.com2F; oraclelicense=accept-securebackup-cookie" "http://download.oracle.com/otn-pub/java/jdk/8u60-b27/jdk-8u60-linux-i586.rpm" -O jdk-8u60-linux-i586.rpm
sudo yum -y localinstall --nogpgcheck jdk-8u60-linux-i586.rpm
